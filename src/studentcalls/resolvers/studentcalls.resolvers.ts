...
> {
  ...
  async userCreated(@Parent() studentCall: StudentCallEntity) {
    const id = studentCall.userCreatedId;
    if (!id) {
      return null;
    }
  ...
  async userUpdated(@Parent() studentCall: StudentCallEntity) {
    const id = studentCall.userUpdatedId;
    if (!id) {
      return null;
    }
